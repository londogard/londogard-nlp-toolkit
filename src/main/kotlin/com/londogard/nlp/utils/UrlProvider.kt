...
object UrlProvider {
    private const val githubDataUrl: String =
        "https://raw.githubusercontent.com/londogard/londogard-nlp-toolkit/main/data"
    private const val bpeUrl: String = "https://nlp.h-its.org/bpemb/"

    val rootPath: Path = Paths.get(System.getProperty("user.home")).resolve(".londogard")
    ...
    fun sentencePiece(language: LanguageSupport, vocabSize: Int): Pair<FileInfo, FileInfo> {
        ...
        val vocab = FileInfo(
            vocabFilename,
            bpePath.resolve(vocabFilename),
            "$baseUrl.vocab",
            "sentencepiece (bpemb) vocab (<10 KB) - $language"
        )
        val model = FileInfo(
            modelFilename,
            bpePath.resolve(modelFilename),
            "$baseUrl.model",
            "sentencepiece (bpemb) model (<4 MB) - $language"
        )
    ...
    fun stopwords(language: LanguageSupport): FileInfo =
        FileInfo(
            language.toString(),
            stopwordPath.resolve(language.toString()),
            "$githubDataUrl/stopwords/$language",
            "stopwords (<2 KB) - $language"
        )
    ...
    fun wordfreq(language: LanguageSupport, size: WordFrequencySize): FileInfo {
        ...
        return FileInfo(
            filename,
            wordFrequencyPath.resolve(filename),
            "$githubDataUrl/wordfreq/$filename",
            "wordfreq (<3 MB) - $language"
        )
    }
